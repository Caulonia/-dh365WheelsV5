    ...
    proccesRegister: (req, res) =>{
     /* const resultaValidation =  validationResult(req);
       ...
       /* const userCreate = req.body;
        let ultimoUser = usuarios.pop();
        usuarios.push(ultimoUser);
        let userNuevo = {
            id: ultimoUser.id +1,
            nombre: userCreate.nombre,
            apellido: userCreate.apellido,
            email: userCreate.email,
            password: bcrypt.hashSync(userCreate.password, 10),
            confirmPassword: bcrypt.hashSync(userCreate.confirmPassword, 10),
            avatar: req.file.filename
        };
        usuarios.push(userNuevo);
        let userNuevoGuardar = JSON.stringify(usuarios,null,2);
        fs.writeFileSync(path.resolve(__dirname,'../dataBase/users.json'), userNuevoGuardar);
